 ...
 * 
 * @author Sudharsan Balausbramani and Dhruv Uppal
 */
...
public class Editor extends JFrame {	
	...
	public void drawSketch(Graphics g) {
		...
		//Shapes in this editor
		for(Integer id: sketch.getMap().navigableKeySet()) {
	...
	private void handlePress(Point p) {
		...
		if(mode == Mode.MOVE) {
			for(int id: sketch.getMap().descendingKeySet()) {
				if(sketch.getMap().get(id).contains((int)p.getX(), (int)p.getY())){
	...
	private void handleDrag(Point p) {
		...
		if(mode == Mode.MOVE && drag) {
			for(int id: sketch.getMap().descendingKeySet()) {
				if(sketch.getMap().get(id).contains(p.x, p.y) && sketch.getMap().get(id) == curr) {
					comm.send("M " + id + " " + (p.x - moveFrom.x) + " " + (p.y - moveFrom.y));
					break;
				}
			}
			moveFrom = p;
